...
- **自更新优先**：每次触发先查 GitHub 有无新版本（第 -1 步），有新版且不覆盖本地经验就先更新再干活；连不上/没装仓库就跳过（fail-open）。
...
技能跑在 `.claude/skills` 的运行副本，源在用户的 git 仓库（push 到 GitHub）。运行副本可能是仓库的拷贝，也可能是指向仓库的 junction/软链——后者运行副本即仓库工作区，`git pull` 完即生效、无需镜像（`update-skill.sh` 会自动识别这种情况）。每次触发先轻量检查 GitHub 有没有新版本，有且安全就先更新——这样总用最新逻辑。fail-open：连不上/没装仓库就静默跳过、用当前版本，别阻塞正事。
...
- 以 "✓" 开头（"已更新到最新…"，含 junction 模式的"无需镜像"变体） → 告诉用户已从 GitHub 更新，并重新读取本 SKILL.md 与 `scripts/`（逻辑可能已变）后再继续。
- "⚠ 本地有未同步改动…"（退出码 2） → 远端有更新，但本地有没回传的改动（多半是累计经验）。**别自动覆盖**，告诉用户、二选一：① 先 `./sync-skill.sh worldcup-bet-advisor "..." --push` 回传再更新（推荐）；② 保留本地、这次跳过更新。

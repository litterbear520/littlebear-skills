...
技能跑在 `.claude/skills` 的运行副本，源在用户的 git 仓库（push 到 GitHub）。每次触发先轻量检查 GitHub 有没有新版本，有且安全就先更新——这样总用最新逻辑。fail-open：连不上/没装仓库就静默跳过、用当前版本，别阻塞正事。
...
- "✓ 已更新到最新并同步到运行副本" → 告诉用户已从 GitHub 更新，并重新读取本 SKILL.md 与 `scripts/`（逻辑可能已变）后再继续。
...
- 结合第 1b 步的真实表现校准：把搜到的真实战况（谁压着打、上一场怎么丢球、是否已被逼平、红牌伤停）和讨论对照——真实表现 > 比分、也常 > 旧讨论；据此校准方向与 `most_likely_scores`，每队综述写进 `form`。

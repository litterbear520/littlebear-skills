...
- **结合复盘**：若经验库/本期复盘指出某模型某类判断更准、某买法倾向屡空，显式据此调整（如"上期让球大胜全空 + DeepSeek 防反更准 → 本期稳健更偏 DeepSeek 的小胜/防反剧本"），并把"复盘怎么影响今天"落进**门面摘要**（见下）让用户看得到——但用大白话、拆成要点，别藏在脑子里、也别堆成一段。

### 门面摘要（`risk_note` + `summary_points`）—— 报告开头第一眼

报告开头那块是**门面**：用户一打开（网页 / PDF）最先看到的就是它。它写给**刚点进来、不懂你内部行话的读者**看，不是你给自己留的复盘草稿。所以一直容易踩两个坑——把 ①②③④ 堆成密不透风的一整段、塞满只有你懂的速记。两条铁律治它：

1. **`risk_note` = 一句话定调**（≤40 字）：今天整体怎么打、最该当心什么，一句说完别展开。
2. **`summary_points` = 2~4 条今日要点**：把定调背后的理由拆成几条**各表一个意思**的大白话短句（防平重点场、让球怎么收、模型怎么采信、博冷思路…），报告渲染成清爽的圆点列表。每条用 `**关键词**：` 起头加粗便于扫读。**别再把推理堆成一坨**——堆叠密集正是要消灭的毛病。

**说人话、别夹黑话**：门面里禁用内部速记，一律翻成普通中文——`favorite 让盘打弱旅`→"强队让球打弱队"、`carry`→"能放心大胜的碾压局"、`over-flag`→"上期点名的那场"、`让-3`→"让 3 球"、`受让不败>押平`→"买『受让方不败』比直接押平更稳"、`去水 / 盘口虚高`→"扣掉抽水后 / 赔率偏高"。能不出现英文就不出现。（注：`**加粗**` 只有门面这两个字段会渲染，正文其它字段写 `**` 会原样漏成字面量，别用。）

**示例（同一份内容：改前一坨 → 改后清爽）**
- 改前：`risk_note` = "复盘驱动：①DeepSeek跌出榜首→以Claude/GLM为锚；②博冷上期over-flag挪威；③激进让球串剔除慢热强队让-3；④今日定调：4场全是favorite让盘打弱旅、无一真carry→重防平、轻让球、别堆串。"
- 改后：
  - `risk_note` = "今天 4 场都是强队让球打弱队，这种盘本届约三成翻车、几乎全是冷平，所以**重点防平、让球收着买、别凑大串**。"
  - `summary_points` = [
    "**防平为先**：哥伦比亚 vs 刚果金最该防平，宁可少赢，也别被冷平清盘。",
    "**让球别贪**：没有放心大胜的碾压局，葡萄牙、英格兰大让球只当小注、不进稳健底仓。",
    "**模型采信**：上期 DeepSeek 押让球太乐观、跌出第一，今天以 Claude、GLM 为主，势均力敌的场才参考 DeepSeek。",
    "**博冷思路**：上期押『大热门被逼平』落空，今天只押摆大巴、专偷分的弱队，不在对攻场押平。"
    ]

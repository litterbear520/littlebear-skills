...
出今天的买法前，先复盘历史、再复盘上一期，把经验沉淀进 `references/experience.md`，再带着经验做今天的分析。**两条线相互独立——没有上一期报告，既不影响、也不跳过历史复盘。**
...
输出 `historical_unreviewed`（已踢完 + 有预测 + 没复盘过的历史场）、`own_run_to_review`（上一期我们自己的 run）、`reviewed_count`（manifest 里已复盘的场数）。**两条线各自独立触发，别互相牵连：**

- `historical_unreviewed` 非空 → 做 **Track 1（历史复盘）**。**这条线和"有没有上一期报告"无关**：首次运行（`reviewed_count` 为 0、`experience.md` 还没建起来）哪怕一份上一期报告都没有，也必须把这些历史场先分析一遍、把经验库建起来；之后每次只增量消化新结束的未复盘场。
- `own_run_to_review` 非空 → 做 **Track 2（上期复盘）**。
- **只有两者都空**，才是真的没东西可复盘，直接进第 0 步。
...
- 复盘两条线独立：只要有"已踢完 + 有预测 + 没复盘过"的历史场就做 Track 1（**首次运行即使没有任何上一期报告，也要先把历史场分析、建起经验库**，子 agent 分治）；Track 2（上期复盘）仅在有自有上一期 run 时做，没有 / 上期未踢完则跳过、报告无复盘模块。复盘过的场次（`reviewed_matches.json` 记录）不重复分析，下次只增量。
- **不构成投注建议**；理性娱乐、量力而行。

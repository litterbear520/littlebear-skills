...
若直连报错/被阻断 → 用 web-access skill 的 CDP proxy 在浏览器里 `fetch` 同样的接口存成本地 JSON，再用脚本的 `--raw-dir` / `--raw-list` / `--raw-detail-dir` 读入（见 data-sources.md 第五节）。**走 CDP 前先展示反爬须知。**
...
- **渐进披露**：顶部三档方案卡常驻；每场只常驻"结论 + 最可能比分 + 价值点 + 重点玩法（胜平负，缺则比分）"，"全部玩法赔率"和"模型讨论对比"默认折叠，点开再看——解决信息过载。
...
**主路径：web-access skill 的 CDP Proxy**（最稳——直连用户日常 Chrome，一条 curl 即开）：

- 先按 web-access skill 跑一次依赖检查确保 Proxy 就绪（已在跑则秒过）：
  ```bash
...
- 新开后台标签页打开报告的本地文件 URL（Windows 用正斜杠）：
  ...
  ```
- 可顺手截图确认渲染正常：`curl -s "http://localhost:3456/screenshot?target=<上一步返回的 targetId>&file=/tmp/r.png"`。

**备选**：web-access 不可用时，用 chrome-devtools MCP（`mcp__chrome-devtools__new_page`；未加载先用 `ToolSearch` 查 `chrome-devtools`）打开同一个 file URL。

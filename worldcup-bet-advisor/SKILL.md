...
- **预测赛前会刷新**：嘉豪站临近开赛会更新各模型的预测正文甚至下注（06-30 实际发生过：首抓旧版被固化进报告、事后返工）。所以第 1 步抓到的不是终稿——生成报告前必须跑第 3c 步重抓核对，变了就重合并、重归一变化条目。
...
- **上期复盘只摆事实**：报告顶部的"上期复盘"把上一期各模型在嘉豪平台的下注（含让球结算）与「最可能比分」，对照终场比分判对错，给出各模型的方向 / 比分命中数。这是**确定性事实呈现**——脚本算、你不判（沿用"汇总不推导"的调性）；它**不构成本站对谁更准的倾向、也不据此调整任何买法**。和上面的"我买的票"结算是两码事（那个追踪你自己的钱、这个看各模型嘴准不准）。详见第 3b 步。
...
`apply` 自带自洽校验：格式非法、或归一比分的**数字**在该模型原话里找不到对应（往往意味着正文已被站点刷新）→ 拒绝写入并逐条列出，修正 `norm_out.json` 后重跑；`⚠` 提醒（朝向与 favor 疑似矛盾、漏归一、brand 拼写对不上）不挡写入，但要回头逐条复核——它拦的正是"唯一人判环节"最容易出的错。
...
## 第 3b 步：上期复盘（生成报告前做，可选）

给报告**最顶部**加一块"上期复盘"：拿上一期各模型的下注 + 猜分，对照终场，看谁押对了。**全是脚本算的确定性事实，你不用判**（只摆事实，见心法）。

1. **定位上一期**：`runs/` 里**比今天早、最近的一个**有 `merged.json` 的日期目录就是"上期"。没有上期（首次跑）→ 跳过本步、报告无复盘模块。
2. **拉终场比分**：复盘要终场结果，用带 `--include-finished` 的赛程索引（第 0 步抓的是"未开赛"，这里要"含已结束"）：
   ```bash
...
3. **算复盘事实**：`build_retro.py` 读上期 merged（各模型 bet / `pred_score_norm` / 让球线）+ 终场，逐场逐模型判**方向**（实际下注是否押中胜平负，让球按 `主队进球 + 让球线` 结算）和**比分**（最可能比分是否完全命中、多个比分命中任一即算），再算各模型命中数、按方向命中排序：
   ...
   python scripts/build_retro.py --prev-merged "runs/<上期>/merged.json" \
     --finals "$WS/finals.json" --date <上期 YYYY-MM-DD> --out "$WS/retro.json"
   ```
   - 它**只复盘已踢完的场**；上期全没踢完 / 一场都对不上终场 → 不写文件、打印提示，报告自动无复盘模块（fail-soft）。
   - 上期部分踢完 → 只复盘已结束那几场，未完的不计入。

## 第 3c 步：赛前新鲜度核对（生成报告前必做）

嘉豪站会在赛前刷新模型预测，第 1 步抓的版本到出报告时可能已是旧版（见心法）。生成报告前重抓核对一次：

```bash
python scripts/fetch_predictions.py verify --against "$WS/predictions.json" --update
```

- `= 全部一致` → 直接进第 4 步。
- `≠ 有变化` → `--update` 已把最新预测原位写回 `predictions.json`。重跑第 2 步 merge；重跑第 3 步归一——未变的条目直接沿用上次 `norm_out.json` 的判读，verify 列出的「场 × 模型」重新判读后合入 `norm_out.json`，再 `apply`。
- 若从出报告到发布站点（第 6 步 push）又隔了较久，push 前不放心可以再 verify 一次——成本只是几次静态 JSON 请求。

## 第 4 步：生成报告

```bash
python scripts/build_report.py --merged "$WS/merged.json" --date <YYYY-MM-DD> \
  --retro "$WS/retro.json" --out "$WS/report.html"   # 第 3b 步没产出 retro.json 就别带 --retro
```

`--date` 传当天日期（报告标题与站点文件名用它；不传则从最早开赛时间推北京日期，遇到深夜场会偏到次日，所以推荐传）。`--retro` 传第 3b 步的 `retro.json`（有则在报告顶部渲染"上期复盘"，没有就省略此参数）。报告纯由 `merged.json`（+ 可选 retro）渲染，**不再需要 analysis.json**。

报告内置 **Anthropic 编辑感**模板，版块顺序固定为：

0. **上期复盘**（仅当传了 `--retro`）：顶部一块"上期各模型准确率"——准确率榜（各模型方向 X/N、比分 Y/N，按方向命中排序，最准的高亮"上期最准"）+ 逐场对错矩阵（每行一场终场比分 + 胜平负，绿圆=方向中、橘菱=比分中、空心=未中，鼠标悬停看该模型那场押了啥猜了啥）。只摆事实。
...
- 上期复盘只判确定性事实（方向/比分对错），无上期或上期没踢完就优雅跳过、报告无此模块；复盘**随 `report.html` 一起**被站点原样嵌入，所以**线上和本地是同一块、不用改两套**（第 6 步发布无需额外动作）。某模型那场没下注/没给比分 → 矩阵里该格显示 `·`、不计入分母。
